...
main() {
    ...
    log_info "  must be stopped first:"
    log_info "    sudo systemctl stop scx && sudo scx_cognis --tui"
    log_info "  Press q or Esc to exit the TUI — the service will NOT restart"
    log_info "  automatically. To bring it back: sudo systemctl start scx"
    echo ""
