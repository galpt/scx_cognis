...
static void dispatch_task(const struct dispatched_task_ctx *task)
{
	 ...
	 *
	 * Count this as a user dispatch: the user-space scheduler made a real
	 * scheduling decision (classify -> slice -> vtime) even though it chose
	 * not to pin the task to a specific core.  Without this increment,
	 * nr_user_dispatches (shown as d->u in --monitor) stays 0 whenever
	 * perf_cri_ema > 0.85 forces all dispatches through RL_CPU_ANY, giving
	 * a misleading "zero user dispatches" reading in the monitor output.
	 */
...
void BPF_STRUCT_OPS(rustland_dispatch, s32 cpu, struct task_struct *prev)
{
	...
	/*
	 * Consume a task from the per-CPU DSQ first.
	 *
	 * Per-CPU kthreads (kworker/N:M, ksoftirqd/N, etc.) are inserted
	 * directly into cpu_to_dsq(cpu) by ops.enqueue and MUST be consumed
	 * before the user-space scheduler is given another slice.  If the
	 * usersched check runs first and nr_scheduled is non-zero (which it
	 * almost always is under any load), ops.dispatch returns immediately
	 * after scheduling the user-space thread, and the per-CPU DSQ is
	 * never drained -- the kworker starves until the sched_ext watchdog
